...
# Reshape the input tensors to have the correct dimensions
x_train = torch.from_numpy(x_train).unsqueeze(1).float()  # Add a channel dimension
y_train = torch.from_numpy(y_train).long()  # labels are typically long
x_test = torch.from_numpy(x_test).unsqueeze(1).float()  # Add a channel dimension
...
class EulerReservoirCell(nn.Module):
    ...
    def __init__(self, units, input_size=6272, activation=nn.Tanh(), weight_decay=0.01):
...
class ReadoutLayer(nn.Module):
    def __init__(self, units):
        ...
        self.fc = nn.Linear(units, 49)  # Assuming 49 is the number of output classes
